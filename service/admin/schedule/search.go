...
func (st StoreSchedule) SearchScheduleByCourseCode(code string) (Schedule, error) {
	scheduleCourse := Schedule{}
	scheduleCourseLT := scheduleCourse.LyThuyet
	ctx := context.Background()

	courseDb, err := models.Courses(
		qm.Where("code = ?", code),
	).One(ctx, st.db)
	if err != nil {
		logrus.WithFields(logrus.Fields{}).Error("[Find Course] error : ", err)
