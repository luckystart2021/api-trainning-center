...
func UnDeleteChildCategory(service child_category.IChildCategoryService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id := chi.URLParam(r, "id")
		if id == "" {
			response.RespondWithError(w, http.StatusBadRequest, errors.New("Mã danh mục không được rỗng"))
		...
		userRole := r.Context().Value("values").(middlewares.Vars)
		deleteCategoryById, err := service.UnDeleteCategoryById(idR, userRole.UserName)
		if err != nil {
			response.RespondWithError(w, http.StatusBadRequest, err)
			return
		}
